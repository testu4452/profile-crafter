...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('profileCrafterApp.error', (response: EventWithContent<unknown>) => {
      const errorResponse = (response as EventWithContent<AlertErrorModel>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = this.eventManager.subscribe('profileCrafterApp.httpError', (response: EventWithContent<unknown>) => {
